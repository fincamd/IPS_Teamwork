...
public class CreateSupplierOrderToSale {
	List<SupplierOrderedProductDto> products;
	int saleId;
	Gateway supplierGateway = (SupplierOrderGateway) PersistenceFactory.createSupplierOrderGateway();
	Gateway orderedProductsGateway = PersistenceFactory.createOrderedProductsGateway();
	Connection con = null;

	public CreateSupplierOrderToSale(List<SupplierOrderedProductDto> products, int saleId) {
	...
	private int createNewOrder() throws SQLException {
		int newOrderId = ((SupplierOrderGateway) supplierGateway).findBiggestId();
		SupplierOrderDto newOrder = new SupplierOrderDto();
		newOrder.id = ++newOrderId;
		newOrder.saleId = this.saleId;
